...
<div class="mail-formatsection">
  <div class="logo"><a href="#"><img src="{{url('images/himalayn_frontiers_logo_vector2017__1_-pdf_720.png')}}"></a></div>
  	@php
        ...
        $activity_price = ($booking->activity_price * $booking->payment_percentage)/100;
        $total_price    = $booking->payment_paid;
    @endphp
  <p>Activity({{$booking->itinerary->title}}) Booked Successfully.</p>
  <br/>
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    <tr>
                        ...
                        <td>{{$booking->currency->symbol}}{{ round($activity_price,2) }}</td>
                        <td>{{$booking->bank_charges}} %</td>
                        <td>{{$booking->booking_status}}</td>
                        <td>{{$booking->currency->symbol}}{{round($total_price,2)}}</td>
                        </tr>
                        <tr>
                        <td colspan="4">Total Payable(Including Bank Charges)</td>
                        <td> {{$booking->currency->symbol}}{{ round( ($total_price) ,2) }}</td>
                    </tr>
                </tbody>
            </table>
