      ...
      this.show = function () {
        ...
        this.showAccordionDialog().then(function (data) {
          ...
          if (data.length > 0) {
            ...
            var toInsert = '<div class="accordion" id="accordionExample">';
            for (var i = 0; i < data.length; i++) {
              var myId = accordionId + (i + 1);
			  toInsert = toInsert + '<div class="card">';
			  toInsert = toInsert + '<div class="card-header" >';
              toInsert = toInsert + '<a class="collapsed card-link" contenteditable="true" data-toggle="collapse" href="#' + myId + '">';
              toInsert = toInsert + data[i].title;
              toInsert = toInsert + '</a>';
              toInsert = toInsert + '</div>';
              toInsert = toInsert + '<div id="' + myId + '" class="collapse" data-parent="#accordion">';
              toInsert = toInsert + '<div class="card-body" contenteditable="true">';
              toInsert = toInsert + data[i].content;
              toInsert = toInsert + '</div>';
              toInsert = toInsert + '</div>';
              toInsert = toInsert + '</div>';
            }
